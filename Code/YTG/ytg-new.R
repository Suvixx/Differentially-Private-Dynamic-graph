...
ytg0 <- read.csv("/media/spaul/My Passport/ytg/g0-full.csv")
ytg1 <- read.csv("/media/spaul/My Passport/ytg/g1-full.csv")
ytg2 <- read.csv("/media/spaul/My Passport/ytg/g2-full.csv")
ytg3 <- read.csv("/media/spaul/My Passport/ytg/g3-full.csv")
...
from <- Reduce(intersect, list(ytg0$from,ytg1$from,ytg2$from,ytg3$from))
to <- Reduce(intersect, list(ytg0$to,ytg1$to,ytg2$to,ytg3$to))
vertex_ytg <- unique(c(from,to))
...
#sampling nodes
sample_node_f <- round(runif(100,0,length(from)))
sample_node_t <- round(runif(100,0,length(to)))
g0_f_v <- ytg0[sample_node_f,1]
g0_t_v <- ytg0[sample_node_t,2]
...
g0_w <- dplyr::inner_join(ytg0,sample_g)
